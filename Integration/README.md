...
Here is also an older guide on running a dedicated Postgres DB with a read-only replica: [link](../GatewayAPI-Full).


## Common use cases
All requests here are made against Radix-run `https://mainnet.radixdlt.com`. While it is suitable for ad-hoc requests and testing the integration, it is strongly advised to run your own Gateway API.
Note that request examples in Postman work with a conventional Gateway API implementation, but `https://mainnet.radixdlt.com` requires a special `X-Radixdlt-Target-Gw-Api` header which is not mentioned in Postman.
It needs to be added, e.g. for *curl*: `-H 'x-radixdlt-target-gw-api: 1.1.2'`.
Alternatively, you can switch to an alternative publicly available API at `https://api.radix.live`.

#### 1. Obtaining the latest state version
https://documenter.getpostman.com/view/14449947/UVXnHaJf#26c0a889-75c4-4a36-9cd3-2d79aa537d99
Here `gateway.ledger_state.version` is the latest version of the Network State in the node's aggregated data, while `gateway.target_ledger_state.version` is the latest version observed on the Network by the Core API node.
...
https://documenter.getpostman.com/view/14449947/UVXnHaJf#2a0fe47c-79af-4c24-9651-33f1e45ebab9
...
Radix account addresses can be validated with RegExp: `^rdx1[02-9AC-HJ-NP-Zac-hj-np-z]{61}$`.

#### 4. Getting account transactions
https://documenter.getpostman.com/view/14449947/UVXnHaJf#c6c7cd44-f093-4298-949c-4f757437e864
Transactions here are ordered by date (newest first) and contain full info (no need to request additional details on each transaction specifically).
...
Encrypting/decrypting private messages is outside of the scope of this doc.

#### 5. Getting transaction details
https://documenter.getpostman.com/view/14449947/UVXnHaJf#645f0327-53b6-439b-bb6f-6bf3fcdb4fcc
...
https://documenter.getpostman.com/view/14449947/UVXnHaJf#efd465b2-939e-4760-860d-969d45c599a0
...
https://redocly.github.io/redoc/?url=https://raw.githubusercontent.com/radixdlt/radixdlt-network-gateway/1.1.1/gateway-api-spec.yaml#tag/Account/paths/~1account~1transactions/post
...
To avoid this, API clients should prevent the possibility of sending more than one transaction at a time from a single account (e.g. enqueue all transactions per account and poll from that queue).
